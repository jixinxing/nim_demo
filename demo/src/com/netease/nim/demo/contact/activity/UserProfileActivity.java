        ...
        if (DemoCache.getAccount() != null && !DemoCache.getAccount().equals(account)) {
            ...
            if (NIMClient.getService(FriendService.class).isMyFriend(account)) {
                RecentContact recentContact = NIMClient.getService(MsgService.class).queryRecentContact(account,
                                                                                                        SessionTypeEnum.P2P);
                boolean isSticky = recentContact != null && CommonUtil.isStickyTagSet(recentContact);
    ...
    private void updateSticky(boolean checkState) {
        ...
        if (checkState) {
            ...
            if (recentContact == null) {
                ...
                NIMClient.getService(MsgService.class).createEmptyRecentContact(account, SessionTypeEnum.P2P, CommonUtil.RECENT_TAG_STICKY, System.currentTimeMillis(), true);
            }
            // 之前存在，更新置顶flag
            else {
